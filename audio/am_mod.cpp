...
static LADSPA_Descriptor * g_psDescriptors = NULL;
...
public:
  ...
  StartupShutdownHandler() {
    ...
    initialise_sine_table();
      ...
      if (g_psDescriptors == NULL)
        ; //break;
        // if this fails then all if fucked right?
      
      g_psDescriptors->UniqueID = 2048;
      g_psDescriptors->Properties = LADSPA_PROPERTY_HARD_RT_CAPABLE;
      // Do I really have to dtrdup this crap?
      g_psDescriptors->Maker = localStrdup("Nikita T");
      ...
      psPortRangeHints[OSC_OUTPUT].HintDescriptor
        = 0;
      g_psDescriptors->instantiate = instantiateSineOscillator;
      g_psDescriptors->connect_port = connectPortToSineOscillator;
      g_psDescriptors->activate = activateSineOscillator;
      g_psDescriptors->run_adding = NULL;
      g_psDescriptors->set_run_adding_gain = NULL;
      g_psDescriptors->deactivate = NULL;
      g_psDescriptors->cleanup = cleanupSineOscillator;
      
        g_psDescriptors->Label = localStrdup("am_mod");
        g_psDescriptors->Name = localStrdup("AM Modulator (Freq:control, Amp:audio)");
        ...
        g_psDescriptors->run = runSineOscillator_FreqCtrl_AmpAudio;
  }
...
// wtf is this function? I don't need it right?
